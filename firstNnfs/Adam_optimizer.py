...
class Adam:
    ...
    def updateParameters(self, layer):
        ...
        layer.weights += -self.currentLearningRate * weight_momentums_corrected / (np.sqrt(weight_cache_corrected) + self.epsilon)
        layer.biases += -self.currentLearningRate * bias_momentums_corrected / (np.sqrt(bias_cache_corrected) + self.epsilon)
